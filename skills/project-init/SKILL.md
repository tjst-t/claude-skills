...
4. Calls `sprint init` from the sprint skill to set up `docs/ROADMAP.json`
...
1. If `docs/ROADMAP.json` already exists, skip this step (it may have been or will be generated by `sprint roadmap`).
2. Otherwise, call `sprint init` from the sprint skill to set up a blank `docs/ROADMAP.json`.
3. If the sprint skill is not available, create `docs/ROADMAP.json` manually with a blank template.
...
└── docs/
    ...
    ├── ROADMAP.json             # Layer 2: Sprint tracking (JSON). Managed by sprint.
