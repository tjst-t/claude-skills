...
3. **sprint roadmap** — VISION + ARCHITECTURE からロードマップを生成(rolling-wave: 最初のマイルストーンまでの近傍 Sprint は詳細に、それ以降は「タイトル + ゴールだけ」の粗い Sprint + backlog として持つ。遠方の詳細は各マイルストーンで just-in-time に詰める)
...
生成されたロードマップを確認する:
- **詳細な初回バッチ**(最初のマイルストーンまで): ストーリーの粒度は適切か、最初のスプリントが「動く最小限」になっているか
- **粗い遠方 Sprint**(それ以降): タイトル + ゴールだけで OK。ここのストーリーはまだ書かない — 各マイルストーンで詰める。弧(全体の流れ)とマイルストーン位置が妥当かだけ見る
- マイルストーンの位置は妥当か

問題があればその場で修正。遠方の Sprint の中身が薄いのは意図的(先に詳細化してもどうせ作り直しになるため)。
