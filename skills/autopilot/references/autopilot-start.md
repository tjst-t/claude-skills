...
7. **Drift check (thin — read recorded results, never re-derive)**: Review the decision log against VISION and DESIGN_PRINCIPLES. If any decision contradicts these documents, flag it but continue (it will be reviewed at the milestone demo). For the done-judgment guards, only confirm the records exist and are conclusive: every Story has a `done_judgment` block in `verification-results.json`, and the machine artifacts (`guards-run.json`, `verify-run.json`) are present for the Sprint. Do NOT re-run guard greps or re-evaluate verdicts here — the guards were evaluated once on machine facts (`sprint verify` Phase 1.7), independently backstopped by the verifier (Phase 1.8), and drift-checked by the machine at the done gate (`sprint-done.md` Step 0.5); a third model pass adds tokens, not trust. A Sprint MISSING these records is the finding — send it back through `sprint verify`. Stories with `overall: needs_user_review` are reported to the milestone summary but do NOT block subsequent Sprint execution — they accumulate as user-review items.
...
4. Present a **milestone summary**:
   ...
   - Backlog items added
   - **Run metrics** — aggregate `docs/sprint-logs/*/run-metrics.json` across the batch into one line per Sprint (agents spawned, review cycles used, caps hit, verifier ran). For every cap hit, list the residual findings the cap deferred (from `decisions.json` / backlog) — the caps trade review rounds for tokens, never visibility, so the user sees at the boundary exactly what was left on the table. Recurring cap-hits or agent-count spikes are efficiency findings for `skill-retrospective.md`, same loop as quality findings.
