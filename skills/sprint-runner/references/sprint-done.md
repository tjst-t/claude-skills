...
6. **Clean up worktrees.** List all git worktrees (`git worktree list`). For each worktree that was created during `sprint run` (typically under `.claude/worktrees/`):
   - Verify that its branch has already been merged into the current branch (`git branch --merged` or check merge commits in `git log`).
   - If merged: remove it with `git worktree remove {path}` and delete the associated branch with `git branch -d {branch}`.
