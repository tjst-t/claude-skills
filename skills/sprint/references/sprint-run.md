...
3. **For each wave (sequentially):**
   ...
   **Step 1 — Implement (sub-agent, sonnet, worktree):**
   ...
   - The agent prompt must include: the Story's Tasks, project context (CLAUDE.md, relevant architecture info), and the instruction to implement all Tasks, run tests, and log output to `docs/sprint-logs/{SprintID}/`. Do NOT paste `docs/VISION.json` / `docs/DESIGN_PRINCIPLES.json` into implement-agent prompts — the plan phase already translated them into the Story's AC and Tasks, and repeating them per agent per wave multiplies their token cost for no decision the agent is allowed to make alone. Include instead the single escalation rule: "If a decision feels load-bearing (data model, new dependency, public API, ADR territory), do not decide — return it as an escalation." Also instruct the agent to report:
     ...
     - **Concerns** (out-of-spec unease): things where the Story's AC are met and no rule is violated, but something still feels off — a flow that reads as a hang, confusing-but-correct copy, an abstraction that points the wrong way. Format: `[{note, theme}]`. This is a **sensor, not a gate**: it never blocks the Story, and an **empty list is a valid, honest answer** — do not manufacture a concern. It is distinct from the backlog item above (that's out-of-scope work; this is "in scope, gates green, but off"). `sprint verify` passes these to the independent verifier, which consolidates them into `verification-report.json`'s `concerns[]`.
   ...
   **Step 3 — Fix (SendMessage to implementation agent, sonnet):**
   ...
   - **Cap: at most 2 review cycles per Story** (the initial Step 2 review + one re-review after fixes). Review findings follow diminishing returns — an unbounded "until no findings remain" loop re-reads the diff every round and mostly polishes. After the cap: apply any remaining auto-fixable findings WITHOUT another review round, log unapplied non-trivial findings to `docs/sprint-logs/{SprintID}/decisions.json`, and record that the cap was hit in `run-metrics.json` (step 4.5).
   - **The cap defers polish, never absorbs defects — and residuals are notified, not buried.** Two hard rules: (1) a residual finding that indicates a **correctness bug** is not deferrable — fix it and prove it with tests (the machine verdict / fix-until-green path, which the cap never applies to), or escalate as `needs_human`; the cap bounds *review rounds*, not *bug fixing*. (2) every deferred residual must reach a human channel: interactive runs list them per Story in the step 5 summary; under autopilot they re-enter `sprint verify` (Phase 2 scope / Phase 2.5 backlog proposals) and the milestone summary's caps-hit line. "Logged to decisions.json" alone does NOT count as notification.
...
4.5. **Record run metrics** — write `docs/sprint-logs/{SprintID}/run-metrics.json` (schema: `references/SPRINT_LOGS_SCHEMA.json`): agents spawned by phase (implement / review), review cycles used per Story, and which Stories hit the review-cycle cap. One small JSON write; it is the data the milestone retrospective uses to see where the token budget actually went and whether the caps are binding.

5. Present a summary of what was implemented. If any Story hit the review-cycle cap, include a **"review cap hit"** section: the Story, the residual findings that were deferred (with one-line dispositions), and where each will resurface (verify Phase 2 / backlog). The user must be able to see what the cap left on the table without reading decisions.json.
