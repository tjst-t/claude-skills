...
`--auto` changes *who resolves an ambiguous choice*, never *what is allowed*. It does NOT relax any gate: the forbidden-action categories (immediate-stop still stops), the 8-guard done judgment, and test-discipline all still apply. Under `--auto`, `sprint verify` additionally enables the independent verifier (`--with-verifier`).
...
- **Two-level review, both bounded**: Story-level review in `sprint run` (fix loop capped at 2 cycles per Story) + Sprint-level review in `sprint verify` (scoped to cross-Story surfaces, re-run at most once). Both are mandatory; neither loops until "no findings remain".
...
- **Test discipline is the source of truth**: All nine rules — every Story has a user scenario, tests drive the user's entry point (CLI subprocess / real HTTP client / real-browser Playwright / library public API), no silent skips, GUI E2E observes UI state through the real backend, status reflects reality, what you ship is what you test (Rule 6: the Sprint's diff scan ensures every implemented user-observable surface has a passing test, not just declared AC), **and 実機検証は本番モードで (Rule 7: priority_rule 9 smoke must run in production mode — no `MOCK=true` / `--fake-*` / `DRY_RUN=1` / in-process Fake)**, plus **Rule 8** (data-path Sprints must carry a destructive multi-version test) and **Rule 9** (test status is machine-derived from real exit codes, never authored by judgment) — live in `references/test-discipline.md`. plan / run / verify / done / auto all defer to it. When something feels ambiguous about what a test must look like, read that file.
...
- **8-Guard Done Judgment**: Before marking any Story as `done` in `sprint verify` / `sprint done` / `sprint auto`, apply all 8 guards defined in `references/sprint-done-judgment.md`. The mechanical guards (2/3/6/7 + configured 5/8 greps) are machine-run once by `hooks/run-guards.py` (`guards-run.json`); the model applies policy on the hits and evaluates the judgment guards (1/4 + applicability of 5/8) — never re-runs the greps by hand. Any failed guard moves the Story to `needs_user_review`, not `done`. Record each guard's pass/fail/warn outcome in `verification-results.json` under `done_judgment`.
- **Machine-derived test status**: `sprint verify` runs the project's declared verification through `hooks/run-verify.py`, which writes a machine-authored verdict (`verify-run.json`) from the real process **exit code** + JUnit. The model copies `status` from that verdict and authors only `evidence` — it never decides pass/fail by reading output. `sprint done`'s gate and an integrity hook block recording a `pass` over a machine failure. This is the fix for the fabrication incident; it is language-agnostic (exit codes + optional JUnit). Full contract: `references/verify-execution.md`.
- **Independent verifier (`--with-verifier`)**: The default `sprint verify` runs in the same session that implemented the code, which grades itself too leniently (self-grading bias). `sprint verify --with-verifier` additionally spawns a **separate read-only Claude session** that re-checks AC against real code, the forbidden-action categories (`test-discipline.md` Rule 6), ADR conformance, and compromise completeness, writing `docs/sprint-logs/{SprintID}/verification-report.json`. That report is the trust source — where it disagrees with the self-report, the verifier wins. `autopilot --auto` always enables it; manual `sprint verify` defaults to off (token cost). Full spec: `references/verifier-agent.md`.
...
- `references/sprint-done-judgment.md` — **canonical 8-guard done judgment** (Guard 1–8) applied before any Story can be marked `done` (the autopilot skill points here; guard 由来 lives in `references/done-judgment-rationale.md`, read only when auditing/revising the guards)
...
- `references/SPRINT_LOGS_SCHEMA.json` — sprint log JSON schemas (decisions, verify-run, guards-run, run-metrics, verification-results, refine, failures, scenario, gui-spec)
- `references/verify-execution.md` — **machine-derived test status** contract: `.claude/verify.json`, the `run-verify.py` wrapper, the `sprint done` machine gate, the integrity hook (language-agnostic; fixes the fabrication incident)
- `references/VERIFY_RUN_SCHEMA.json` — `verify-run.json` schema (machine-authored test verdict)
